...
class Robot:
    ...
    def next_action_training(self, state):
        ...
        if(self.train_step_counter < 500):
            ...
            # self.object_timer += 1
            self.object_dist = np.linalg.norm(state - self.goal_state)
    ...
    def next_action_testing(self, state):
        ...
        if(self.needs_planning):
            ...
            print(self.object_dist)
            if(self.object_dist > 0.3):
                self.planning_cross_entropy(state,planning_horizon = 175, K = 0.03, N = 75, iterations = 5)
            else:
